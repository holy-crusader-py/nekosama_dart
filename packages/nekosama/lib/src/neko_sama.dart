...
class NekoSama {
  ...
  final Map<String, String> headers = {
    "Referer": "https://animecat.net/",
    "Origin": "https://animecat.net/",
  };

  /// The main api for the `nekosama` library.
  NekoSama({HttpClient? httpClient, Map<String, String>? headers})
  ...
  Future<NSHome> getHome() async {
    final homePageResponse = await Uri.https("animecat.net", "")
        .get(httpClient: httpClient, headers: headers);
  ...
  Future<NSAnime> getAnime(Uri url) async {
    try {
      final request = await url.get(
        httpClient: httpClient,
        headers: headers,
      );
      final animePageBody = request.body;
      ...
      final synopsis = synopsisRaw == null ||
              synopsisRaw
                  .toLowerCase()
                  .contains("aucun synopsis pour le moment")
          ? null
          : synopsisRaw;
      ...
      return NSAnime(
        ...
        genres: [
          for (final e
              in document.getElementsByClassName("tag list").first.children)
            NSGenres.fromString(
              RegExp(r"\[\W+([\w\s\-]+)\W")
        ...
        status: NSStatuses.fromString(
              RegExp(r"^\s\w+(.+)")
                      .firstMatch(infos?.elementAt(2).text ?? "")
                      ?.group(1) ??
                  "",
            ) ??
            NSStatuses.aired,
        type: NSTypes.fromString(
              RegExp(r"^\s\w+(.+)")
                      .firstMatch(infos?.elementAt(1).text ?? "")
                      ?.group(1) ??
                  "",
            ) ??
            NSTypes.tv,
  ...
  Future<List<Uri>?> guessEpisodeUrls(NSAnimeBase anime) async {
    ...
    if (anime.episodeCount == 0) {
      ...
      return eps == null ? null : [...eps.map((e) => e.url)];
    }
    final baseUrl = anime.url.toString().replaceFirst("/info/", "/episode/");
    return [
  ...
  ///
  /// Returns `null` if no episodes were found.
  Future<List<NSEpisode>?> getEpisodes(NSAnimeBase anime,
  ...
  Future<List<NSEpisode>?> _getEpisodes(int animeId, Uri animeUrl,
      [String? animePageBody]) async {
    try {
      final rawEps = RegExp(r"(?<=var\sepisodes\s=\s)\[.+\](?=;)")
          .firstMatch(animePageBody ??
              (await animeUrl.get(httpClient: httpClient)).body)
          ?.group(0);
      if (rawEps == null) {
        return null;
      }
      ...
      throw NekoSamaException(
          "Failed to parse episodes for anime at url '$animeUrl'", e);
    }
  }
  ...
  Future<List<Uri>> getVideoUrls(NSEpisode episode) async {
    final Uri videoUri = Uri.parse(
      episode.url.toString().replaceFirst("neko-sama.fr", "animecat.net"),
    );
    final episodePageBody =
        (await videoUri.get(httpClient: httpClient, headers: headers)).body;
      ...
      return [
        ...RegExp(
          ...
          multiLine: true,
            ...
            .allMatches(episodePageBody)
            .map(
              (e) => Uri.tryParse(
                (e.group(1)?.isNotEmpty ?? true)
                    ? e.group(1)!
                    : "::Not valid URI::",
              ),
            )
  ...
  Future<List<NSSearchAnime>> getSearchDb(
    ...
    try {
      ...
      throw NekoSamaException(
          "Failed to build a NSSearchAnime from the search db", e);
    }
  }
